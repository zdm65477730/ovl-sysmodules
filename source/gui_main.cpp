...
GuiMain::GuiMain() {
    ...
    if (this->m_sysmodEnabledFlags.sysmodulesControlEnabled) {
        ...
        for (const auto &entry : FsDirIterator(contentDir)) {
            ...
            if (sysmoduleProgramId == 0x420000000007E51AULL || sysmoduleProgramId == 0x420000000000000BULL)
                continue;
    ...
    if (this->m_sysmodEnabledFlags.wifiControlEnabled) {
        ...
        this->m_listItemWifiSwitch->setClickListener([this](u64 click) -> bool {
            if (click == HidNpadButton_A) {
                Result rc;
                bool isWifiOn = false;
                if (R_FAILED(rc = nifmIsWirelessCommunicationEnabled(&isWifiOn)))
                    this->m_listItemWifiSwitch->setValue("WifiSwitchStatusCheckErrorListItemText"_tr + std::to_string(rc));
                else {
